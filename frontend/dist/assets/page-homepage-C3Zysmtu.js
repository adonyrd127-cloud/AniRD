import{X as z}from"./vendor-DIPEJTOH.js";class M{constructor(){this.tokenKey="anird_auth_token",this.userKey="anird_user",this.host=window.location.hostname||"localhost",this.baseUrl=`http://${this.host}:3005/api/v1/auth`,this.userUrl=`http://${this.host}:3005/api/v1/user`}getToken(){return localStorage.getItem(this.tokenKey)}getUser(){const e=localStorage.getItem(this.userKey);return e?JSON.parse(e):null}isLoggedIn(){return!!this.getToken()}async login(e,i){try{const r=await(await fetch(`${this.baseUrl}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:e,password:i})})).json();if(!r.success)throw new Error(r.message);return localStorage.setItem(this.tokenKey,r.token),localStorage.setItem(this.userKey,JSON.stringify(r.user)),r}catch(t){throw console.error("Fetch error:",t),t instanceof TypeError?new Error(`Error de conexión al servidor (${this.baseUrl}).`):t}}async register(e,i){try{const r=await(await fetch(`${this.baseUrl}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:e,password:i})})).json();if(!r.success)throw new Error(r.message);return localStorage.setItem(this.tokenKey,r.token),localStorage.setItem(this.userKey,JSON.stringify(r.user)),r}catch(t){throw console.error("Fetch error:",t),t instanceof TypeError?new Error(`Error de conexión al servidor (${this.baseUrl}). Asegúrate de que el puerto 3005 esté abierto.`):t}}logout(){localStorage.removeItem(this.tokenKey),localStorage.removeItem(this.userKey);try{console.log("[Auth] Borrando base de datos IndexedDB local AniRD_DB al cerrar sesión...");const e=indexedDB.deleteDatabase("AniRD_DB"),i=()=>{window.location.href="/"};e.onsuccess=i,e.onerror=i,e.onblocked=i,setTimeout(i,800)}catch(e){console.error("[Auth] Error borrando DB:",e),window.location.href="/"}}async syncWithServer(e){return this.isLoggedIn()?await(await fetch(`${this.userUrl}/sync`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.getToken()}`},body:JSON.stringify(e),keepalive:!0})).json():null}async fetchFromServer(){if(!this.isLoggedIn())return null;const i=await(await fetch(`${this.userUrl}/sync`,{headers:{Authorization:`Bearer ${this.getToken()}`}})).json();return i.success?i.syncData:null}}const T=new M,n=new z("AniRD_DB");n.version(3).stores({history:"++id, animeId, episodeId, progress, duration, timestamp, updatedAt",favorites:"animeId, title, cover, addedAt",following:"animeId, title, cover, broadcast, addedAt",lists:"++id, name, animeIds, createdAt",cache:"key, data, expiresAt",settings:"key, value",notifications:"++id, animeId, isRead, timestamp"});n.open().catch(async s=>{console.error("[Dexie] Error crítico al abrir la base de datos AniRD_DB:",s);try{console.log("[Dexie] Intentando restablecer base de datos local para auto-recuperación..."),await z.delete("AniRD_DB"),console.log("[Dexie] Base de datos borrada con éxito. Recargando página...")}catch(e){console.error("[Dexie] Fallo al borrar base de datos:",e)}window.location.reload()});const B={async triggerSync(){try{if(T.isLoggedIn()){const s=await this.getAllData();await T.syncWithServer(s),console.log("[Sync] Sincronización en la nube exitosa.")}}catch(s){console.error("[Sync] Error sincronizando con el servidor:",s)}},async addToHistory(s,e,i,t,r={}){const u=Date.now(),m=await n.history.where({animeId:s,episodeId:e}).first();let g;return m?g=await n.history.update(m.id,{progress:i,duration:t,updatedAt:u,...r}):g=await n.history.add({animeId:s,episodeId:e,progress:i,duration:t,timestamp:u,updatedAt:u,...r}),this.triggerSync(),g},async getContinueWatching(){const s=await n.history.orderBy("updatedAt").reverse().toArray(),e=new Map;return s.forEach(t=>{e.has(t.animeId)||e.set(t.animeId,t)}),Array.from(e.values()).filter(t=>!t.duration||t.duration===0?!0:t.progress/t.duration*100<90).slice(0,20)},async toggleFavorite(s){var t,r;const e=s.mal_id||s.id||s.animeId;return await n.favorites.get(e)?(await n.favorites.delete(e),this.triggerSync(),!1):(await n.favorites.add({animeId:e,title:s.title,cover:((r=(t=s.images)==null?void 0:t.jpg)==null?void 0:r.large_image_url)||s.cover||"",type:s.type||"",score:s.score||"",episodes:s.episodes||null,status:s.status||"",broadcast:s.broadcast||null,addedAt:Date.now()}),this.triggerSync(),!0)},async isFavorite(s){return s?!!await n.favorites.get(Number(s)):!1},async getFavorites(){return await n.favorites.orderBy("addedAt").reverse().toArray()},async toggleFollowing(s){var t,r;const e=s.mal_id||s.id||s.animeId;return await n.following.get(e)?(await n.following.delete(e),this.triggerSync(),!1):(await n.following.add({animeId:e,title:s.title,cover:((r=(t=s.images)==null?void 0:t.jpg)==null?void 0:r.large_image_url)||s.cover||"",status:s.status||"",broadcast:s.broadcast||null,addedAt:Date.now(),lastNotified:Date.now()}),this.triggerSync(),!0)},async isFollowing(s){return s?!!await n.following.get(Number(s)):!1},async getFollowing(){return await n.following.orderBy("addedAt").reverse().toArray()},async getSetting(s,e=null){const i=await n.settings.get(s);return i?i.value:e},async setSetting(s,e){return await n.settings.put({key:s,value:e})},async getAllData(){return{favorites:await n.favorites.toArray(),following:await n.following.toArray(),history:await n.history.toArray(),lists:await n.lists.toArray()}},async syncFromServer(s){if(s)return await n.transaction("rw",[n.favorites,n.following,n.history,n.lists],async()=>{const e=await n.history.toArray(),i=s.history||[],t=new Map;e.forEach(o=>{const a=`${o.animeId}_${o.episodeId}`;t.set(a,o)});let r=!1;i.forEach(o=>{const a=`${o.animeId}_${o.episodeId}`,c=t.get(a);if(!c)t.set(a,o);else{const d=o.updatedAt||o.timestamp||0,l=c.updatedAt||c.timestamp||0;d>l?t.set(a,o):l>d&&(r=!0)}}),e.length!==t.size&&(r=!0);const u=await n.favorites.toArray(),m=s.favorites||[],g=new Map;u.forEach(o=>g.set(Number(o.animeId),o)),m.forEach(o=>{const a=Number(o.animeId),c=g.get(a);if(!c)g.set(a,o);else{const d=o.addedAt||0,l=c.addedAt||0;d>l?g.set(a,o):l>d&&(r=!0)}}),u.length!==g.size&&(r=!0);const y=await n.following.toArray(),w=s.following||[],h=new Map;y.forEach(o=>h.set(Number(o.animeId),o)),w.forEach(o=>{const a=Number(o.animeId),c=h.get(a);if(!c)h.set(a,o);else{const d=o.addedAt||0,l=c.addedAt||0;d>l?h.set(a,o):l>d&&(r=!0)}}),y.length!==h.size&&(r=!0);const b=await n.lists.toArray(),v=s.lists||[],p=new Map;b.forEach(o=>p.set(o.id,o)),v.forEach(o=>{const a=o.id,c=p.get(a);if(!c)p.set(a,o);else{const d=o.createdAt||0,l=c.createdAt||0;d>l?p.set(a,o):l>d&&(r=!0)}}),b.length!==p.size&&(r=!0),await n.history.clear();const f=Array.from(t.values()).map(o=>{const{id:a,...c}=o;return c});await n.history.bulkAdd(f),await n.favorites.clear(),await n.favorites.bulkAdd(Array.from(g.values())),await n.following.clear(),await n.following.bulkAdd(Array.from(h.values())),await n.lists.clear(),await n.lists.bulkAdd(Array.from(p.values())),r&&(console.log("[Sync] Detectados cambios locales más recientes. Subiendo fusión al servidor..."),setTimeout(()=>this.triggerSync(),0))})}},R=Object.freeze(Object.defineProperty({__proto__:null,db:n,dbService:B},Symbol.toStringTag,{value:"Module"}));class C{constructor(){this.baseUrl="https://api.jikan.moe/v4",this.lastRequest=0,this.minDelay=500,this.cache=new Map,this.inflight=new Map,this.cacheTTL=10*60*1e3}async request(e,i={},t={}){const r=new URL(`${this.baseUrl}${e}`);Object.keys(i).forEach(y=>r.searchParams.append(y,i[y]));const u=r.toString(),m=this.cache.get(u);if(m&&m.expires>Date.now())return m.data;if(this.inflight.has(u))return this.inflight.get(u);const g=(async()=>{const y=Date.now(),w=Math.max(0,this.lastRequest+this.minDelay-y);w>0&&await new Promise(v=>setTimeout(v,w)),this.lastRequest=Date.now();let h=await fetch(u,{signal:t.signal});if(h.status===429&&(await new Promise(v=>setTimeout(v,2e3)),h=await fetch(u,{signal:t.signal})),!h.ok)throw new Error(`Jikan error: ${h.status}`);const b=await h.json();return this.cache.set(u,{data:b,expires:Date.now()+this.cacheTTL}),this.inflight.delete(u),b})();return this.inflight.set(u,g),g}}class D{constructor(){this.baseUrl="https://graphql.anilist.co"}async request(e,i={}){const t=await fetch(this.baseUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:e,variables:i})});if(!t.ok)throw new Error(`AniList error: ${t.status}`);return(await t.json()).data}}class q{constructor(){const e=window.location.hostname||"localhost";this.port=3005,this.baseUrl=`http://${e}:${this.port}/api/v1`,this.apiKey="dev-anime1v-key"}async request(e,i={}){const t=new URL(`${this.baseUrl}${e}`);Object.keys(i).forEach(r=>t.searchParams.append(r,i[r]));try{const r=await fetch(t.toString(),{headers:{"X-API-Key":this.apiKey}});if(!r.ok)throw new Error(`Local API error: ${r.status}`);return r.json()}catch(r){return console.error(`Error en petición local a ${e}:`,r),{success:!1,message:r.message}}}}class j{constructor(){this.providers={jikan:new C,anilist:new D,local:new q},this.cache=new Map}async getAnimeSearch(e,i={}){return await this.providers.jikan.request("/anime",{q:e,limit:20},i)}async searchLocal(e){if(!e)return{success:!1,data:{results:[]}};try{let i=await this.providers.local.request("/anime/search",{q:e});if((!i.success||!i.data.results.length)&&e.length>5){const t=e.split(/[:\(\-]|Season|Movie|Part/i)[0].trim();t!==e&&(i=await this.providers.local.request("/anime/search",{q:t}))}if(!i.success||!i.data.results.length){const t=e.split(" ")[0];t.length>3&&(i=await this.providers.local.request("/anime/search",{q:t}))}return i}catch{return{success:!1,data:{results:[]}}}}async getAnimeInfo(e){try{if(typeof e=="string"&&(e.includes("http")||e.includes("anime/")))return await this.providers.local.request("/anime/info",{url:e});if(this.cache.has(e))return this.cache.get(e);const i=await this.providers.jikan.request(`/anime/${e}/full`);return this.cache.set(e,i),i}catch{return{success:!1,data:null}}}async getEpisode(e){return await this.providers.local.request("/anime/episode",{url:e})}async getTrending(e=1){return await this.providers.jikan.request("/top/anime",{filter:"airing",limit:24,page:e})}async getMovies(e=1){return await this.providers.jikan.request("/top/anime",{type:"movie",filter:"bypopularity",limit:24,page:e})}async getLatest(e=1){return await this.providers.jikan.request("/seasons/now",{limit:24,page:e})}async getDubbed(e=1){try{return await this.providers.jikan.request("/anime",{producers:"1191,108",limit:24,page:e,order_by:"popularity",sort:"desc"})}catch{return{data:[]}}}async getByGenre(e,i=1){return await this.providers.jikan.request("/anime",{genres:e,order_by:"popularity",limit:24,page:i})}async getSchedule(){return await this.providers.jikan.request("/seasons/now")}async getAnimeRelations(e){return await this.providers.jikan.request(`/anime/${e}/relations`)}async getAnimeCharacters(e){return await this.providers.jikan.request(`/anime/${e}/characters`)}async getAnilistBanner(e){var t;const i="query ($id: Int) { Media (idMal: $id, type: ANIME) { bannerImage } }";try{return(t=(await this.providers.anilist.request(i,{id:e})).Media)==null?void 0:t.bannerImage}catch{return null}}async getAnimeRecommendations(e){return await this.providers.jikan.request(`/anime/${e}/recommendations`)}async getAnilistEpisodes(e){var t;const i=`
      query ($id: Int) {
    ...
    `}render(){var v,p,f,o,a,c;if(!this._anime)return;const e=this.getAttribute("mode")==="thumbnail",i=((p=(v=this._anime.images)==null?void 0:v.webp)==null?void 0:p.large_image_url)||((o=(f=this._anime.images)==null?void 0:f.jpg)==null?void 0:o.large_image_url)||((c=(a=this._anime.images)==null?void 0:a.jpg)==null?void 0:c.image_url)||this._anime.image||this._anime.thumbnail||"",t=this._anime.title_english||this._anime.title||"Anime",r=this._anime.mal_id||this._anime.id,u=this._anime.score||this._anime.rating||"?.?",m=this._anime.type||"TV",g=this._anime.episodes?`${this._anime.episodes} eps`:"",y=this._anime.progress||0,w=this._anime.duration_watched||0,h=w>0?Math.min(y/w*100,100):0,b=this._anime.status==="Currently Airing";this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          width: ${e?"280px":"185px"};
      ...
      <a href="/anime/${r}" data-link class="card-inner">
        <div class="img-container">
          <img src="${i}" alt="${t}" loading="lazy" referrerpolicy="no-referrer">
        </div>
    ...
    `).join("")}async afterRender(){const e=document.getElementById("hero-container-v4"),i=document.getElementById("trending-grid"),t=document.getElementById("movies-grid"),r=document.getElementById("continue-grid"),u=document.getElementById("continue-section"),m=document.getElementById("api-error-banner"),g=document.getElementById("api-error-msg");let y=!1;const w=async(h,b,v)=>{try{const p=await h();return p&&p.data&&p.data.length>0?b&&this._renderGrid(b,p.data):(b&&(b.innerHTML=`<p style="color:rgba(255,255,255,0.3);padding:20px;font-size:13px;">Sin resultados para ${v}</p>`),y=!0),p}catch(p){return console.error(`[AniRD] Error cargando ${v}:`,p),b&&(b.innerHTML=`<p style="color:rgba(255,100,100,0.5);padding:20px;font-size:13px;">Error al cargar ${v}</p>`),y=!0,null}};try{const h=document.getElementById("latest-grid"),b=document.getElementById("latino-grid"),v=document.getElementById("action-grid"),[p]=await Promise.all([w(()=>x.getTrending(),i,"Populares"),w(()=>x.getMovies(),t,"Películas"),w(()=>x.getLatest(),h,"Últimos"),w(()=>x.getDubbed(),b,"Latino"),w(()=>x.getByGenre("1,2"),v,"Acción")]);if(y&&m&&(m.style.display="flex",g&&(g.textContent="Algunas secciones no pudieron cargar. Verifica que el servidor esté activo.")),p&&p.data&&p.data.length>0){const f=p.data.slice(0,5),o=await Promise.all(f.map(d=>x.getAnilistBanner(d.mal_id).then(l=>{var k,A;return l||((A=(k=d.images)==null?void 0:k.jpg)==null?void 0:A.large_image_url)}).catch(()=>{var l,k;return(k=(l=d.images)==null?void 0:l.jpg)==null?void 0:k.large_image_url})));let a=0;const c=d=>{var _,I,$;const l=f[d];if(!l)return;const k=o[d]||((I=(_=l.images)==null?void 0:_.jpg)==null?void 0:I.large_image_url)||"";let A=f.map((E,S)=>`<button class="indicator ${S===d?"active":""}" onclick="window.setHeroIndex(${S})"></button>`).join("");window.setHeroIndex=E=>{a=E,c(a),clearInterval(this._heroInterval),this._heroInterval=setInterval(()=>{a=(a+1)%f.length,c(a)},7e3)},e.innerHTML=`
            <div class="hero-v5">
        ...
        `;try{const f=await B.getContinueWatching();if(f&&f.length>0){u.style.display="block";const o=f.slice(0,8);await Promise.all(o.map(async a=>{try{const c=document.createElement("anime-card");if(c.setAttribute("mode","thumbnail"),r.appendChild(c),a.animeTitle&&a.animeCover)c.data={mal_id:a.animeId,title:a.animeTitle,images:{jpg:{large_image_url:a.animeCover}},type:a.animeType||"",score:a.animeScore||"",currentEpisode:a.episodeId,progress:a.progress||0,duration_watched:a.duration||0},x.getAnimeInfo(a.animeId).then(d=>{d!=null&&d.data&&(c.data={...d.data,currentEpisode:a.episodeId,progress:a.progress||0})}).catch(()=>{});else{const d=await x.getAnimeInfo(a.animeId);d!=null&&d.data&&(c.data={...d.data,currentEpisode:a.episodeId,progress:a.progress||0})}}catch{}}))}}catch{}}catch(h){console.error("[AniRD HomePage] Error general:",h),m&&(m.style.display="flex",g&&(g.textContent=`Error de carga: ${h.message}. Verifica que el servidor esté activo en el puerto correcto.`))}}destroy(){this._heroInterval&&(clearInterval(this._heroInterval),this._heroInterval=null)}_renderGrid(e,i){!e||!i||(e.innerHTML="",i.forEach(t=>{const r=document.createElement("anime-card");r.data=t,e.appendChild(r)}))}}const U=Object.freeze(Object.defineProperty({__proto__:null,default:N},Symbol.toStringTag,{value:"Module"}));export{U as H,x as a,n as b,T as c,B as d,R as e,H as f};
