import{g as E,u as C}from"./page-watchpage-CF4pbcAd.js";import{a as M,b as c,c as f,d as y}from"./page-homepage-gz9FGPn7.js";import{s as p}from"./page-profilepage-BIS2M2Yy.js";import{T as v}from"./page-animedetailpage-AOBgfgm5.js";import"./vendor-DIPEJTOH.js";import"./dependencies-Ce00LDPN.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function o(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=o(n);fetch(n.href,i)}})();class T{constructor(t){this.router=t,this.timeout=null,this.abortController=null,this.render(),this.bindEvents()}render(){this.container=document.createElement("div"),this.container.className="search-overlay",this.container.id="searchOverlay",this.container.innerHTML=`
      <style>
        .search-overlay {
          position: fixed;
          inset: 0;
    ...
    `,document.body.appendChild(this.container),this.input=this.container.querySelector("#palette-input"),this.resultsContainer=this.container.querySelector("#palette-results"),this.clearBtn=this.container.querySelector("#palette-clear"),this.exploreStateHtml=this.container.querySelector("#explore-state").outerHTML}bindEvents(){document.addEventListener("keydown",t=>{(t.metaKey||t.ctrlKey)&&t.key==="k"&&(t.preventDefault(),this.open()),t.key==="Escape"&&this.container.classList.contains("active")&&this.close()}),this.container.addEventListener("click",t=>{t.target===this.container&&this.close()}),this.clearBtn.addEventListener("click",()=>{this.input.value="",this.input.focus(),this.clearBtn.classList.remove("visible"),this.resultsContainer.innerHTML=this.exploreStateHtml}),this.input.addEventListener("input",t=>{const o=t.target.value.trim();o.length>0?this.clearBtn.classList.add("visible"):this.clearBtn.classList.remove("visible"),this.abortController&&this.abortController.abort(),this.abortController=new AbortController;const a=this.abortController.signal;if(o.length===0){this.resultsContainer.innerHTML=this.exploreStateHtml;return}this.resultsContainer.innerHTML=`
      ...
      `,this.timeout=setTimeout(async()=>{try{const n=await M.getAnimeSearch(o,{signal:a});if(a.aborted)return;const i=(n.data||[]).slice(0,10);i.length>0?(this.resultsContainer.innerHTML=`
              <div class="result-group">
                ${i.map(r=>{var l,s;return`
                  <a href="/anime/${r.mal_id||r.id}" data-link class="result-item search-result-item">
                    <img src="${((s=(l=r.images)==null?void 0:l.jpg)==null?void 0:s.image_url)||r.image||""}" alt="">
                    ...
                    <div class="result-item-genres">
                      ${(r.genres||[]).slice(0,2).map(d=>`<span class="genre-badge">${d.name}</span>`).join("")}
                    </div>
                  </a>
                `}).join("")}
              </div>
            ...
            `}catch(n){if(n.name==="AbortError")return}},400)}),this.input.addEventListener("keydown",t=>{if(t.key==="Enter"){const o=this.input.value.trim();o&&(this.close(),this.router.navigate(`/search?q=${encodeURIComponent(o)}`))}})}open(){this.container.classList.add("active"),this.input.focus()}close(){this.container.classList.remove("active")}}const g={async checkNewEpisodes(){const e=await c.following.toArray(),t=await c.favorites.toArray(),o=new Map;t.forEach(i=>o.set(i.animeId,i)),e.forEach(i=>o.set(i.animeId,i));const a=Array.from(o.values());let n=!1;for(const i of a)if(i.status==="Currently Airing"&&i.broadcast&&i.broadcast.day&&i.broadcast.day!=="Unknown"){const r=this.getLastBroadcast(i.broadcast);r&&(!i.lastNotified||r>i.lastNotified)&&(await c.notifications.where("animeId").equals(i.animeId).and(u=>u.timestamp===r).first()||(await c.notifications.add({animeId:i.animeId,isRead:0,timestamp:r}),n=!0),await c.favorites.get(i.animeId)&&await c.favorites.update(i.animeId,{lastNotified:r}),await c.following.get(i.animeId)&&await c.following.update(i.animeId,{lastNotified:r}))}return n},getLastBroadcast(e){const t={Sundays:0,Mondays:1,Tuesdays:2,Wednesdays:3,Thursdays:4,Fridays:5,Saturdays:6};if(t[e.day]===void 0)return null;const[o,a]=e.time.split(":").map(Number),n=new Date(new Date().toLocaleString("en-US",{timeZone:e.timezone||"Asia/Tokyo"}));let i=new Date(n);i.setHours(o,a,0,0);let r=t[e.day]-n.getDay();(r<0||r===0&&i<=n)&&(r+=7),i.setDate(i.getDate()+r);const l=new Date(i);if(l.setDate(l.getDate()-7),l>n)return null;const s=l.getTime()-n.getTime();return Date.now()+s},async getUnreadCount(){return await c.notifications.where("isRead").equals(0).count()},async getNotifications(){const e=await c.notifications.orderBy("timestamp").reverse().toArray(),t=[];for(const o of e){let a=await c.following.get(o.animeId);a||(a=await c.favorites.get(o.animeId)),a&&t.push({...o,title:a.title,cover:a.cover})}return t},async markAllAsRead(){await c.notifications.where("isRead").equals(0).modify({isRead:1})}},L=document.getElementById("app"),I=E(L),z=new T(I),B=document.createElement("header");B.innerHTML=`
...
`;document.body.insertBefore(B,L);const S=()=>{const e=document.getElementById("profile-link"),t=document.getElementById("mobile-profile-link"),o=document.getElementById("sidebar-profile-link"),a=document.getElementById("profile-avatar-container"),n=document.getElementById("profile-username-text"),i=f.isLoggedIn(),r=f.getUser(),l=i?"/profile":"/auth";if(e&&(e.setAttribute("href",l),i?(a&&a.classList.add("active-border"),n&&(n.textContent=(r==null?void 0:r.username)||"Perfil")):(a&&a.classList.remove("active-border"),n&&(n.textContent="Entrar"))),t){t.setAttribute("href",l);const s=t.querySelector(".nav-label");s&&(s.textContent=i?"Perfil":"Entrar")}if(o){o.setAttribute("href",l);const s=o.querySelector("span");s&&(s.textContent=i?"Mi Perfil":"Entrar")}},A=e=>{const t=document.getElementById("bottomNav");if(!t)return;const o=t.querySelectorAll(".nav-item");o.forEach(n=>{n.classList.remove("active");const i=n.querySelector("svg");i&&i.setAttribute("stroke-width","1.8")});let a=-1;if(e==="/"||e.startsWith("/anime/")||e.startsWith("/category/")||e.startsWith("/watch/")||e==="/calendar"?a=0:e==="/favorites"?a=2:e==="/lists"?a=3:(e==="/profile"||e==="/auth")&&(a=4),a!==-1&&o[a]){o[a].classList.add("active");const n=o[a].querySelector("svg");n&&n.setAttribute("stroke-width","2.2")}};C.subscribe(e=>{A(e.currentRoute),p.isActive&&setTimeout(()=>{p.updateFocusables(),p.focusFirstAvailable()},400)});window.updateNavbarAuth=S;S();A(window.location.pathname);const x=e=>{e&&e.preventDefault(),z.open()},b=document.getElementById("desktop-search-trigger");b&&b.addEventListener("click",x);const w=document.getElementById("open-search-btn");w&&w.addEventListener("click",x);const k=document.getElementById("mobile-search-btn");k&&k.addEventListener("click",x);const D=async()=>{const e=document.getElementById("nav-notifications"),t=document.getElementById("notif-badge"),o=document.getElementById("notif-dropdown"),a=document.getElementById("notif-list"),n=document.getElementById("mark-read-btn");if(!e)return;const i=async()=>{await g.getUnreadCount()>0?(t.style.display="block",e.style.color="#ff0000"):(t.style.display="none",e.style.color="inherit")},r=async()=>{const s=await g.getNotifications();if(s.length===0){a.innerHTML=`
      ...
      `;return}a.innerHTML=s.slice(0,8).map(d=>`
      <a href="/anime/${d.animeId}" data-link style="display: flex; gap: 12px; padding: 12px 10px; text-decoration: none; border-bottom: 1px solid rgba(255,255,255,0.04); border-radius: 12px; transition: background 0.2s; ${d.isRead?"opacity: 0.5;":""}" onmouseenter="this.style.background='rgba(255,255,255,0.04)'" onmouseleave="this.style.background='transparent'">
        <img src="${d.cover}" style="width: 44px; height: 60px; object-fit: cover; border-radius: 8px; flex-shrink: 0;" loading="lazy">
        <div style="flex: 1; min-width: 0;">
          <div style="color: white; font-size: 12px; font-weight: 700; margin-bottom: 3px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; font-family: 'Inter', sans-serif;">${d.title}</div>
          <div style="color: var(--accent); font-size: 10px; font-weight: 800; display: flex; align-items: center; gap: 4px;">
            <span style="width: 6px; height: 6px; border-radius: 50%; background: var(--accent); ${d.isRead?"opacity: 0;":""}"></span>
            Nuevo episodio
          </div>
          <div style="color: var(--text-muted); font-size: 9px; margin-top: 4px; font-weight: 500;">${l(d.timestamp)}</div>
        </div>
      </a>
    `).join("")};function l(s){const d=Date.now()-s,u=Math.floor(d/6e4);if(u<1)return"Ahora mismo";if(u<60)return`Hace ${u} min`;const h=Math.floor(u/60);if(h<24)return`Hace ${h}h`;const m=Math.floor(h/24);return m<7?`Hace ${m}d`:new Date(s).toLocaleDateString()}if(e.addEventListener("click",async s=>{if(s.target===n)return;const d=o.style.display==="block";o.style.display=d?"none":"block",d||await r()}),n.addEventListener("click",async s=>{s.stopPropagation(),await g.markAllAsRead(),await i(),await r(),v.success("Notificaciones leídas","Se marcaron todas como leídas")}),document.addEventListener("click",s=>{e.contains(s.target)||(o.style.display="none")}),f.isLoggedIn()&&await g.checkNewEpisodes()){const d=await g.getUnreadCount();d>0&&v.notify("Nuevos episodios",`Tienes ${d} episodio${d>1?"s":""} nuevo${d>1?"s":""} disponible${d>1?"s":""}`,{label:"Ver",onClick:()=>{e.click()}})}await i()},N=()=>{const e=document.getElementById("tv-mode-toggle"),t=document.getElementById("tv-mode-text"),o=document.getElementById("header-tv-toggle"),a=l=>{t&&(t.textContent=l?"📺 Modo TV: ON":"📺 Modo TV: OFF"),e&&(l?e.classList.add("active"):e.classList.remove("active")),o&&(l?(o.classList.add("active"),o.style.background="rgba(255, 0, 85, 0.2)",o.style.boxShadow="0 0 15px rgba(255, 0, 85, 0.4)",o.style.border="1px solid rgba(255, 0, 85, 0.4)"):(o.classList.remove("active"),o.style.background="rgba(255,255,255,0.05)",o.style.boxShadow="none",o.style.border="none"))},n=()=>{p.isActive?(p.destroy(),a(!1)):(p.init(),a(!0))};e&&e.addEventListener("click",l=>{l.preventDefault(),n()}),o&&o.addEventListener("click",l=>{l.preventDefault(),n()});const i=localStorage.getItem("tvMode")==="true",r=(/AniRD-AndroidTV|SmartTV|GoogleTV|AppleTV|HbbTV|LG NetCast|Opera TV|Tizen|Web0S|Nexus Player|AndroidTV|Roku|AFT|Silk|FireTV|Amazon|Chromecast|DroidTV|TV\s+Box|Smart_TV|MiBox|Shield/i.test(navigator.userAgent)||navigator.userAgent.toLowerCase().includes("tv"))&&!navigator.userAgent.includes("AniRD-AndroidMobile");i||localStorage.getItem("tvMode")===null&&r?(p.init(),a(!0)):a(!1)},j=async()=>{let e=await y.getSetting("theme",null);if(e||(e=window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"),e==="light"?document.body.classList.add("light-theme"):document.body.classList.remove("light-theme"),f.isLoggedIn())try{const t=await f.fetchFromServer();t&&await y.syncFromServer(t)}catch{console.error("Sync fail")}await D(),setTimeout(N,200)};j();"serviceWorker"in navigator&&navigator.serviceWorker.register("/sw.js").catch(console.warn);window.AniRDBridge={isPlaying:()=>{const e=document.body.classList.contains("player-active")||document.querySelector(".watch-container")!==null,t=document.querySelector("video")!==null,o=document.querySelector("#video-container iframe, iframe")!==null;return e&&(t||o)},enterPip:()=>{document.body.classList.add("pip-mode")},exitPip:()=>{document.body.classList.remove("pip-mode")},play:()=>{const e=document.querySelector("video");if(e)e.play().catch(t=>console.error("Play error:",t));else{const t=document.querySelector("iframe");t&&t.contentWindow&&t.contentWindow.postMessage(JSON.stringify({event:"command",func:"playVideo"}),"*")}},pause:()=>{const e=document.querySelector("video");if(e)e.pause();else{const t=document.querySelector("iframe");t&&t.contentWindow&&t.contentWindow.postMessage(JSON.stringify({event:"command",func:"pauseVideo"}),"*")}},togglePlayPause:()=>{const e=document.querySelector("video");e&&(e.paused?e.play().catch(t=>console.error("Play error:",t)):e.pause())},nextEpisode:()=>{const e=document.querySelector('.next-episode-btn, #next-ep-btn, button[title*="Siguiente"], a[title*="Siguiente"]');e&&e.click()},prevEpisode:()=>{const e=document.querySelector('.prev-episode-btn, #prev-ep-btn, button[title*="Anterior"], a[title*="Anterior"]');e&&e.click()},seekForward:e=>{const t=document.querySelector("video");t&&(t.currentTime=Math.min(t.duration||0,t.currentTime+e))},seekBack:e=>{const t=document.querySelector("video");t&&(t.currentTime=Math.max(0,t.currentTime-e))},handleBack:()=>{const e=document.querySelector('.modal.active, .dropdown.open, #notif-dropdown[style*="block"]');return e?(e.id==="notif-dropdown"?e.style.display="none":e.classList.remove("active","open"),!0):!1},loginWithToken:e=>{e&&f.loginWithToken(e).then(()=>{window.updateNavbarAuth&&window.updateNavbarAuth()}).catch(t=>console.error("Auto login token error:",t))},notifyPlayback:(e,t)=>{window.Android&&window.Android.onPlaybackChanged&&window.Android.onPlaybackChanged(e,t)}};setInterval(()=>{const e=document.querySelector("video");if(e&&!e.dataset.hasAniRDListener){e.dataset.hasAniRDListener="true";const t=()=>{const o=document.querySelector(".watch-anime-title, .anime-title, h1, h2"),a=o?o.textContent.trim():"AniRD Video";window.AniRDBridge&&window.AniRDBridge.notifyPlayback&&window.AniRDBridge.notifyPlayback(a,!e.paused)};e.addEventListener("play",t),e.addEventListener("pause",t),e.addEventListener("ended",()=>{window.AniRDBridge&&window.AniRDBridge.notifyPlayback&&window.AniRDBridge.notifyPlayback("AniRD",!1)})}},1e3);
