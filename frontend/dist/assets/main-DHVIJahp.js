import{g as E,u as C}from"./page-watchpage-D0BtXkCg.js";import{a as T,b as c,c as f,d as y}from"./page-homepage-hw3Gvt4E.js";import{s as p}from"./page-profilepage-BVT-XFyh.js";import{T as v}from"./page-animedetailpage-L2pJW0ER.js";import"./vendor-DIPEJTOH.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function o(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(n){if(n.ep)return;n.ep=!0;const i=o(n);fetch(n.href,i)}})();class I{constructor(t){this.router=t,this.timeout=null,this.abortController=null,this.render(),this.bindEvents()}render(){this.container=document.createElement("div"),this.container.className="search-overlay",this.container.id="searchOverlay",this.container.innerHTML=`
      <style>
        .search-overlay {
          position: fixed;
          inset: 0;
    ...
    `,document.body.appendChild(this.container),this.input=this.container.querySelector("#palette-input"),this.resultsContainer=this.container.querySelector("#palette-results"),this.clearBtn=this.container.querySelector("#palette-clear"),this.exploreStateHtml=this.container.querySelector("#explore-state").outerHTML}bindEvents(){document.addEventListener("keydown",t=>{(t.metaKey||t.ctrlKey)&&t.key==="k"&&(t.preventDefault(),this.open()),t.key==="Escape"&&this.container.classList.contains("active")&&this.close()}),this.container.addEventListener("click",t=>{t.target===this.container&&this.close()}),this.clearBtn.addEventListener("click",()=>{this.input.value="",this.input.focus(),this.clearBtn.classList.remove("visible"),this.resultsContainer.innerHTML=this.exploreStateHtml}),this.input.addEventListener("input",t=>{const o=t.target.value.trim();o.length>0?this.clearBtn.classList.add("visible"):this.clearBtn.classList.remove("visible"),this.abortController&&this.abortController.abort(),this.abortController=new AbortController;const a=this.abortController.signal;if(o.length===0){this.resultsContainer.innerHTML=this.exploreStateHtml;return}this.resultsContainer.innerHTML=`
      ...
      `,this.timeout=setTimeout(async()=>{try{const n=await T.getAnimeSearch(o,{signal:a});if(a.aborted)return;const i=(n.data||[]).slice(0,10);i.length>0?(this.resultsContainer.innerHTML=`
...
`;document.body.insertBefore(L,B);const S=()=>{const e=document.getElementById("profile-link"),t=document.getElementById("mobile-profile-link"),o=document.getElementById("sidebar-profile-link"),a=document.getElementById("profile-avatar-container"),n=document.getElementById("profile-username-text"),i=f.isLoggedIn(),r=f.getUser(),l=i?"/profile":"/auth";if(e&&(e.setAttribute("href",l),i?(a&&a.classList.add("active-border"),n&&(n.textContent=(r==null?void 0:r.username)||"Perfil")):(a&&a.classList.remove("active-border"),n&&(n.textContent="Entrar"))),t){t.setAttribute("href",l);const s=t.querySelector(".nav-label");s&&(s.textContent=i?"Perfil":"Entrar")}if(o){o.setAttribute("href",l);const s=o.querySelector("span");s&&(s.textContent=i?"Mi Perfil":"Entrar")}},A=e=>{const t=document.getElementById("bottomNav");if(!t)return;const o=t.querySelectorAll(".nav-item");o.forEach(n=>{n.classList.remove("active");const i=n.querySelector("svg");i&&i.setAttribute("stroke-width","1.8")});let a=-1;if(e==="/"||e.startsWith("/anime/")||e.startsWith("/category/")||e.startsWith("/watch/")||e==="/calendar"?a=0:e==="/favorites"?a=2:e==="/lists"?a=3:(e==="/profile"||e==="/auth")&&(a=4),a!==-1&&o[a]){o[a].classList.add("active");const n=o[a].querySelector("svg");n&&n.setAttribute("stroke-width","2.2")}};C.subscribe(e=>{A(e.currentRoute),p.isActive&&setTimeout(()=>{p.updateFocusables(),p.focusFirstAvailable()},400)});window.updateNavbarAuth=S;S();A(window.location.pathname);const m=e=>{e&&e.preventDefault(),z.open()},b=document.getElementById("desktop-search-trigger");b&&b.addEventListener("click",m);const w=document.getElementById("open-search-btn");w&&w.addEventListener("click",m);const k=document.getElementById("mobile-search-btn");k&&k.addEventListener("click",m);const D=async()=>{const e=document.getElementById("nav-notifications"),t=document.getElementById("notif-badge"),o=document.getElementById("notif-dropdown"),a=document.getElementById("notif-list"),n=document.getElementById("mark-read-btn");if(!e)return;const i=async()=>{await g.getUnreadCount()>0?(t.style.display="block",e.style.color="#ff0000"):(t.style.display="none",e.style.color="inherit")},r=async()=>{const s=await g.getNotifications();if(s.length===0){a.innerHTML=`
