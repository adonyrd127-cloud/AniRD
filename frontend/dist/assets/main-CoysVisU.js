import{g as T,u as A}from"./page-watchpage-DTYwnab-.js";import{a as N,b as d,d as g,c as u}from"./page-homepage-HfrhAw2J.js";import{s as c}from"./page-profilepage-DJntT8s1.js";import"./vendor-DIPEJTOH.js";(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(a){if(a.ep)return;a.ep=!0;const r=t(a);fetch(a.href,r)}})();class M{constructor(i){this.router=i,this.timeout=null,this.render(),this.bindEvents()}render(){this.container=document.createElement("div"),this.container.className="search-overlay",this.container.id="searchOverlay",this.container.innerHTML=`
      <style>
        .search-overlay {
          position: fixed;
          inset: 0;
    ...
    `,document.body.appendChild(this.container),this.input=this.container.querySelector("#palette-input"),this.resultsContainer=this.container.querySelector("#palette-results")}bindEvents(){document.addEventListener("keydown",i=>{(i.metaKey||i.ctrlKey)&&i.key==="k"&&(i.preventDefault(),this.open()),i.key==="Escape"&&this.container.classList.contains("active")&&this.close()}),this.container.addEventListener("click",i=>{i.target===this.container&&this.close()}),this.input.addEventListener("input",i=>{const t=i.target.value.trim();if(clearTimeout(this.timeout),t.length<3){this.resultsContainer.innerHTML=`
      ...
      `,this.timeout=setTimeout(async()=>{try{const a=((await N.getAnimeSearch(t)).data||[]).slice(0,8);a.length>0?(this.resultsContainer.innerHTML=`
              <div class="result-group">
                <h4>Animes</h4>
          ...
          `}},400)}),this.input.addEventListener("keydown",i=>{if(i.key==="Enter"){const t=this.input.value.trim();t&&(this.close(),this.router.navigate(`/search?q=${encodeURIComponent(t)}`))}})}open(){this.container.classList.add("active"),this.input.focus()}close(){this.container.classList.remove("active"),this.input.value=""}}const p={async checkNewEpisodes(){const e=await d.following.toArray();let i=!1;for(const t of e)if(t.status==="Currently Airing"&&t.broadcast&&t.broadcast.day&&t.broadcast.day!=="Unknown"){const s=this.getLastBroadcast(t.broadcast);s&&(!t.lastNotified||s>t.lastNotified)&&(await d.notifications.where("animeId").equals(t.animeId).and(r=>r.timestamp===s).first()||(await d.notifications.add({animeId:t.animeId,isRead:0,timestamp:s}),i=!0),await d.following.update(t.animeId,{lastNotified:s}))}return i},getLastBroadcast(e){const i={Sundays:0,Mondays:1,Tuesdays:2,Wednesdays:3,Thursdays:4,Fridays:5,Saturdays:6};if(i[e.day]===void 0)return null;const[t,s]=e.time.split(":").map(Number),a=new Date(new Date().toLocaleString("en-US",{timeZone:e.timezone||"Asia/Tokyo"}));let r=new Date(a);r.setHours(t,s,0,0);let o=i[e.day]-a.getDay();(o<0||o===0&&r<=a)&&(o+=7),r.setDate(r.getDate()+o);const n=new Date(r);if(n.setDate(n.getDate()-7),n>a)return null;const l=n.getTime()-a.getTime();return Date.now()+l},async getUnreadCount(){return await d.notifications.where("isRead").equals(0).count()},async getNotifications(){const e=await d.notifications.orderBy("timestamp").reverse().toArray(),i=[];for(const t of e){const s=await d.following.get(t.animeId);s&&i.push({...t,title:s.title,cover:s.cover})}return i},async markAllAsRead(){await d.notifications.where("isRead").equals(0).modify({isRead:1})}},h=document.getElementById("app"),z=T(h),j=new M(z),L=document.createElement("header");L.innerHTML=`
...
`;document.body.insertBefore(L,h);const m=document.createElement("aside");m.className="desktop-sidebar";m.innerHTML=`
...
`;document.body.insertBefore(m,h);const B=()=>{const e=document.getElementById("profile-link"),i=document.getElementById("mobile-profile-link"),t=document.getElementById("sidebar-profile-link"),s=document.getElementById("profile-avatar-container"),a=document.getElementById("profile-username-text"),r=u.isLoggedIn(),o=u.getUser(),n=r?"/profile":"/auth";if(e&&(e.setAttribute("href",n),r?(s&&s.classList.add("active-border"),a&&(a.textContent=(o==null?void 0:o.username)||"Perfil")):(s&&s.classList.remove("active-border"),a&&(a.textContent="Entrar"))),i){i.setAttribute("href",n);const l=i.querySelector(".nav-label");l&&(l.textContent=r?"Perfil":"Entrar")}if(t){t.setAttribute("href",n);const l=t.querySelector("span");l&&(l.textContent=r?"Mi Perfil":"Entrar")}},C=e=>{const i=document.getElementById("bottomNav");if(!i)return;const t=i.querySelectorAll(".nav-item");t.forEach(a=>{a.classList.remove("active");const r=a.querySelector("svg");r&&r.setAttribute("stroke-width","1.8")});let s=-1;if(e==="/"||e.startsWith("/anime/")||e.startsWith("/category/")||e.startsWith("/watch/")||e==="/calendar"?s=0:e==="/favorites"?s=2:(e==="/profile"||e==="/auth")&&(s=3),s!==-1&&t[s]){t[s].classList.add("active");const a=t[s].querySelector("svg");a&&a.setAttribute("stroke-width","2.2")}},S=e=>{const i=document.querySelector(".desktop-sidebar");if(!i)return;i.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active"));let s="/";e==="/"||e.startsWith("/anime/")||e.startsWith("/watch/")||e.startsWith("/category/")||e==="/calendar"?s="/":e==="/favorites"?s="/favorites":e==="/history"||e==="/my-anird"?s="/history":(e==="/profile"||e==="/auth")&&(s="/profile");const a=i.querySelector(`.sidebar-link[data-route="${s}"]`);a&&a.classList.add("active")},I=async()=>{const e=document.getElementById("sidebar-following-list");if(e)try{const i=await g.getFollowing();if(i.length===0){e.innerHTML=`
        <div class="sidebar-following-empty">
          Aún no sigues ningún anime. Haz clic en "Seguir" en la ficha del anime para agregarlo aquí.
        </div>
      `;return}const t=await d.history.orderBy("updatedAt").reverse().toArray(),s=new Map;t.forEach(a=>{s.has(a.animeId)||s.set(a.animeId,a.episodeId)}),e.innerHTML=i.slice(0,5).map(a=>{const r=Number(a.animeId),o=s.get(r),n=o?`Ep. ${o}`:"Ver ahora";return`
      ...
      `}).join("")}catch(i){console.error("Error al renderizar siguiendo en la barra lateral:",i)}};A.subscribe(e=>{C(e.currentRoute),S(e.currentRoute),I(),c.isActive&&setTimeout(()=>{c.updateFocusables(),c.focusFirstAvailable()},400)});window.updateNavbarAuth=B;B();C(window.location.pathname);S(window.location.pathname);I();const v=e=>{e&&e.preventDefault(),j.open()},y=document.getElementById("desktop-search-trigger");y&&y.addEventListener("click",v);const b=document.getElementById("open-search-btn");b&&b.addEventListener("click",v);const k=document.getElementById("mobile-search-btn");k&&k.addEventListener("click",v);const x=document.getElementById("sidebar-search-btn");x&&x.addEventListener("click",v);const f=document.getElementById("sidebar-categories-trigger"),w=document.getElementById("sidebar-categories-dropdown");f&&w&&f.addEventListener("click",e=>{e.preventDefault(),f.classList.toggle("expanded"),w.classList.toggle("expanded")});const E=document.getElementById("sidebar-collapse-btn");E&&(localStorage.getItem("sidebarCollapsed")==="true"&&document.body.classList.add("sidebar-collapsed"),E.addEventListener("click",i=>{i.preventDefault();const t=document.body.classList.toggle("sidebar-collapsed");localStorage.setItem("sidebarCollapsed",t?"true":"false")}));const D=async()=>{const e=document.getElementById("nav-notifications"),i=document.getElementById("notif-badge"),t=document.getElementById("notif-dropdown"),s=document.getElementById("notif-list"),a=document.getElementById("mark-read-btn");if(!e)return;const r=async()=>{await p.getUnreadCount()>0?(i.style.display="block",e.style.color="#ff0000"):(i.style.display="none",e.style.color="inherit")},o=async()=>{const n=await p.getNotifications();if(n.length===0){s.innerHTML='<div style="color:var(--text-muted); font-size:12px; text-align:center; padding: 20px 0;">No hay notificaciones nuevas</div>';return}s.innerHTML=n.map(l=>`
    ...
    `).join("")};e.addEventListener("click",async n=>{if(n.target===a)return;const l=t.style.display==="block";t.style.display=l?"none":"block",l||await o()}),a.addEventListener("click",async n=>{n.stopPropagation(),await p.markAllAsRead(),await r(),await o()}),document.addEventListener("click",n=>{e.contains(n.target)||(t.style.display="none")}),u.isLoggedIn()&&await p.checkNewEpisodes(),await r()},q=()=>{const e=document.getElementById("tv-mode-toggle"),i=document.getElementById("tv-mode-text"),t=document.getElementById("header-tv-toggle"),s=n=>{i&&(i.textContent=n?"📺 Modo TV: ON":"📺 Modo TV: OFF"),e&&(n?e.classList.add("active"):e.classList.remove("active")),t&&(n?(t.classList.add("active"),t.style.background="rgba(255, 0, 85, 0.2)",t.style.boxShadow="0 0 15px rgba(255, 0, 85, 0.4)",t.style.border="1px solid rgba(255, 0, 85, 0.4)"):(t.classList.remove("active"),t.style.background="rgba(255,255,255,0.05)",t.style.boxShadow="none",t.style.border="none"))},a=()=>{c.isActive?(c.destroy(),s(!1)):(c.init(),s(!0))};e&&e.addEventListener("click",n=>{n.preventDefault(),a()}),t&&t.addEventListener("click",n=>{n.preventDefault(),a()});const r=localStorage.getItem("tvMode")==="true",o=/SmartTV|GoogleTV|AppleTV|HbbTV|LG NetCast|Opera TV|Tizen|Web0S|Nexus Player|AndroidTV|Roku|AFT|Silk|FireTV|Amazon/i.test(navigator.userAgent);r||localStorage.getItem("tvMode")===null&&o?(c.init(),s(!0)):s(!1)},$=async()=>{if(await g.getSetting("theme","dark")==="light"&&document.body.classList.add("light-theme"),u.isLoggedIn())try{const i=await u.fetchFromServer();i&&await g.syncFromServer(i)}catch{console.error("Sync fail")}await D(),setTimeout(q,200)};$();
