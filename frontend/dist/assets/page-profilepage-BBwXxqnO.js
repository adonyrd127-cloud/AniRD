...
import{_ as b}from"./page-watchpage-BEyhp2v9.js";import{c as g,b as w,d as h}from"./page-homepage-HfrhAw2J.js";class S{constructor(){this.isActive=!1,this.focusedElement=null,this.focusableElements=[],this.handleKeyDownBound=this.handleKeyDown.bind(this),this.mutationObserver=null}init(){this.isActive||(this.isActive=!0,document.body.classList.add("tv-mode"),localStorage.setItem("tvMode","true"),window.addEventListener("keydown",this.handleKeyDownBound,{capture:!0}),this.setupMutationObserver(),this.lockWindowScroll(),setTimeout(()=>{this.updateFocusables(),this.focusFirstAvailable()},300),console.log("📺 AniRD Spatial Navigation (Smart TV Mode) initialized."))}destroy(){if(!this.isActive)return;this.isActive=!1,document.body.classList.remove("tv-mode"),localStorage.setItem("tvMode","false"),window.removeEventListener("keydown",this.handleKeyDownBound,{capture:!0}),this.unlockWindowScroll(),this.mutationObserver&&(this.mutationObserver.disconnect(),this.mutationObserver=null),this.focusedElement&&(this.focusedElement.classList.remove("focused"),this.focusedElement=null),document.querySelectorAll(".focused").forEach(r=>r.classList.remove("focused")),console.log("📺 AniRD Spatial Navigation (Smart TV Mode) destroyed.")}lockWindowScroll(){this.scrollListener=()=>{(window.scrollY!==0||window.scrollX!==0||document.documentElement.scrollTop!==0||document.body.scrollTop!==0)&&(window.scrollTo(0,0),document.documentElement.scrollTop=0,document.body.scrollTop=0)},window.addEventListener("scroll",this.scrollListener,{passive:!0}),window.scrollTo(0,0),document.documentElement.scrollTop=0,document.body.scrollTop=0}unlockWindowScroll(){this.scrollListener&&(window.removeEventListener("scroll",this.scrollListener,{passive:!0}),this.scrollListener=null)}setupMutationObserver(){this.mutationObserver&&this.mutationObserver.disconnect(),this.mutationObserver=new MutationObserver(()=>{this.updateTimeout&&clearTimeout(this.updateTimeout),this.updateTimeout=setTimeout(()=>{this.isActive&&this.updateFocusables()},200)}),this.mutationObserver.observe(document.body,{childList:!0,subtree:!0})}updateFocusables(){if(!this.isActive)return;const e=["a[href]","button",'input:not([type="hidden"])',"select","textarea",'[tabindex]:not([tabindex="-1"])',"anime-card",".anime-card",".episode-btn",".sidebar-link",".search-bar-desktop",".header-profile-btn","#header-tv-toggle",".server-btn",".player-control-btn",".category-item",".sidebar-categories-trigger",".control-btn-v5",".server-pill-v5",".lang-pill-v5",".btn-more-v5",".ep-search-input-v5",".sidebar-icon-btn",".ep-item-horizontal-v5",".related-card-v5",".video-wrapper-v5",".tab-item",'[role="button"]'].join(", "),r=Array.from(document.querySelectorAll(e));this.focusableElements=r.filter(d=>{if(d.disabled||d.getAttribute("aria-disabled")==="true")return!1;if(d.id==="tv-mode-toggle"||d.id==="header-tv-toggle")return!0;const f=d.getBoundingClientRect();if(f.width===0||f.height===0)return!1;const s=window.getComputedStyle(d);if(s.display==="none"||s.visibility==="hidden"||s.opacity==="0")return!1;let n=d.parentElement;for(;n;){if(window.getComputedStyle(n).display==="none")return!1;n=n.parentElement}return!0}),this.focusedElement&&!this.focusableElements.includes(this.focusedElement)&&(this.focusedElement.classList.remove("focused"),this.focusedElement=null,this.focusFirstAvailable())}focusFirstAvailable(){if(this.focusableElements.length>0){let e=this.focusableElements.find(r=>r.classList.contains("sidebar-link")||r.tagName==="ANIME-CARD"||r.classList.contains("ep-item-horizontal-v5"));e||(e=this.focusableElements[0]),this.focusElement(e)}}focusElement(e){if(e){if(this.focusedElement&&(this.focusedElement.classList.remove("focused"),typeof this.focusedElement.blur=="function"&&this.focusedElement.blur()),this.focusedElement=e,this.focusedElement.classList.add("focused"),typeof this.focusedElement.focus=="function")try{this.focusedElement.focus({preventScroll:!0})}catch{this.focusedElement.focus()}this.focusedElement.scrollIntoView({behavior:"smooth",block:"nearest",inline:"nearest"})}}handleKeyDown(e){var f;if(!this.isActive)return;const r=document.activeElement?document.activeElement.tagName:"",d=r==="INPUT"||r==="TEXTAREA";if(["ArrowUp","ArrowDown","ArrowLeft","ArrowRight"].includes(e.key)){if(d&&["ArrowLeft","ArrowRight"].includes(e.key))return;e.preventDefault(),e.stopPropagation();const s=e.key.replace("Arrow","").toLowerCase();this.moveFocus(s);return}if(e.key==="Enter"){if(d)return;if(this.focusedElement)if(e.preventDefault(),e.stopPropagation(),console.log("📺 TV D-pad Select: Enter key triggered on element:",this.focusedElement),this.focusedElement.tagName==="ANIME-CARD"){const s=(f=this.focusedElement.shadowRoot)==null?void 0:f.querySelector("a");s?s.click():this.focusedElement.click()}else if(this.focusedElement.classList.contains("video-wrapper-v5")){const s=this.focusedElement.querySelector("iframe");if(s){console.log("📺 TV D-pad: Transferring focus to video iframe."),s.focus();return}}else this.focusedElement.click();return}if(e.key==="Backspace"||e.key==="Escape"){if(d&&e.key==="Backspace")return;const s=document.getElementById("searchOverlay");if(s&&s.classList.contains("active"))return;e.preventDefault(),e.stopPropagation(),console.log("📺 TV D-pad Back: navigating history back"),window.history.back()}}moveFocus(e){if(this.updateFocusables(),this.focusableElements.length===0)return;if(!this.focusedElement||!this.focusableElements.includes(this.focusedElement)){this.focusFirstAvailable();return}const r=this.focusedElement.getBoundingClientRect(),d=r.left+r.width/2,f=r.top+r.height/2;let s=null,n=1/0;for(const c of this.focusableElements){if(c===this.focusedElement)continue;const t=c.getBoundingClientRect(),a=t.left+t.width/2,o=t.top+t.height/2,i=a-d,l=o-f;let u=0,m=0,p=!1;switch(e){case"right":i>5&&(u=i,m=Math.abs(l),p=!0);break;case"left":i<-5&&(u=-i,m=Math.abs(l),p=!0);break;case"down":l>5&&(u=l,m=Math.abs(i),p=!0);break;case"up":l<-5&&(u=-l,m=Math.abs(i),p=!0);break}if(p){const v=u+m*3;v<n&&(n=v,s=c)}}s?this.focusElement(s):console.log(`📺 No focusable candidate found in direction: ${e}`)}}const x=new S;class T{constructor(e){this.params=e,this.user=g.getUser(),this.activeTab="list",this.followedAnimes=[],this.favoriteAnimes=[],this.stats={favs:0,list:0,epis:0}}async render(){if(!this.user)return window.location.href="/auth",document.createElement("div");this.followedAnimes=await w.following.toArray(),this.favoriteAnimes=await w.favorites.toArray(),this.stats={favs:this.favoriteAnimes.length,list:this.followedAnimes.length,epis:await w.history.count()};const e=document.createElement("div");return e.className="page-container",e.innerHTML=`
    ...
    `,e}async afterRender(){const e=document.getElementById("profile-tab-content"),r=document.querySelectorAll(".tab-btn-v5"),d=async s=>{if(this.activeTab=s,r.forEach(n=>n.classList.toggle("active",n.dataset.tab===s)),s==="list"){let c=function(t){if(!t||!t.day||!t.time||!t.timezone)return null;const o={Sundays:0,Mondays:1,Tuesdays:2,Wednesdays:3,Thursdays:4,Fridays:5,Saturdays:6}[t.day];if(o===void 0)return null;const[i,l]=t.time.split(":").map(Number),u=new Date(new Date().toLocaleString("en-US",{timeZone:t.timezone}));let m=new Date(u);m.setHours(i,l,0,0);let p=o-u.getDay();(p<0||p===0&&m<u)&&(p+=7),m.setDate(m.getDate()+p);const v=m-u;if(v<=0)return null;const E=Math.floor(v/(1e3*60*60*24)),y=Math.floor(v/(1e3*60*60)%24);return E>0?`${E}d ${y}h para nuevo cap.`:y>0?`${y}h para nuevo cap.`:"¡Nuevo cap en breve!"};e.innerHTML='<div class="grid-v5" id="grid-follow"></div>';const n=document.getElementById("grid-follow");this.followedAnimes.length>0?this.followedAnimes.forEach(t=>{const a=document.createElement("anime-card"),o={mal_id:t.animeId,title:t.title,images:{jpg:{large_image_url:t.cover}}};a.data=o,n.appendChild(a),Promise.all([b(async()=>{const{db:i}=await import("./page-homepage-HfrhAw2J.js").then(l=>l.e);return{db:i}},__vite__mapDeps([0,1])).then(({db:i})=>i.history.where("animeId").equals(String(t.animeId)).reverse().sortBy("timestamp")),b(async()=>{const{apiService:i}=await import("./page-homepage-HfrhAw2J.js").then(l=>l.f);return{apiService:i}},__vite__mapDeps([0,1])).then(({apiService:i})=>i.getAnimeInfo(t.animeId))]).then(([i,l])=>{let u={...o};if(i&&i.length>0&&(u.currentEpisode=i[0].episodeId),l&&l.data)if(l.data.status==="Currently Airing"){const m=c(l.data.broadcast);m&&(u.nextEpisodeText=m),u.status="EN EMISIÓN"}else l.data.status==="Finished Airing"&&(u.status="FINALIZADO");a.data=u})}):e.innerHTML='<p style="color:var(--text-muted)">Tu lista de seguimiento está vacía.</p>'}if(s==="favs"){e.innerHTML='<div class="grid-v5" id="grid-favs"></div>';const n=document.getElementById("grid-favs");this.favoriteAnimes.length>0?this.favoriteAnimes.forEach(c=>{const t=document.createElement("anime-card"),a={mal_id:c.animeId,title:c.title,images:{jpg:{large_image_url:c.cover}},status:c.status,broadcast:c.broadcast};t.data=a,n.appendChild(t),b(async()=>{const{apiService:o}=await import("./page-homepage-HfrhAw2J.js").then(i=>i.f);return{apiService:o}},__vite__mapDeps([0,1])).then(({apiService:o})=>{o.getAnimeInfo(c.animeId).then(i=>{i&&i.data&&(t.data={...a,status:i.data.status,broadcast:i.data.broadcast,episodes:i.data.episodes,score:i.data.score})})})}):e.innerHTML='<p style="color:var(--text-muted)">Aún no tienes favoritos.</p>'}if(s==="settings"){const n=await h.getSetting("theme","dark"),c=await h.getSetting("audio_pref","sub");e.innerHTML=`
        ...
        `,document.getElementById("audio-pref").addEventListener("change",async t=>await h.setSetting("audio_pref",t.target.value)),document.getElementById("theme-pref").addEventListener("change",async t=>{const a=t.target.value;await h.setSetting("theme",a),document.body.classList.toggle("light-theme",a==="light")}),document.getElementById("tv-mode-pref").addEventListener("change",t=>{const a=t.target.value==="on";a?x.init():x.destroy();const o=document.getElementById("header-tv-toggle");o&&(a?(o.classList.add("active"),o.style.background="rgba(255, 0, 85, 0.2)",o.style.boxShadow="0 0 15px rgba(255, 0, 85, 0.4)",o.style.border="1px solid rgba(255, 0, 85, 0.4)"):(o.classList.remove("active"),o.style.background="rgba(255,255,255,0.05)",o.style.boxShadow="none",o.style.border="none"))}),document.getElementById("sync-btn").addEventListener("click",async t=>{t.target.textContent="Procesando...";const a=await h.getAllData();await g.syncWithServer(a),window.location.reload()}),document.getElementById("reset-db-btn").addEventListener("click",async t=>{if(confirm("¿Estás seguro de que deseas restablecer la base de datos local? Tu historial y favoritos se descargarán de nuevo de la nube de forma limpia.")){t.target.textContent="Restableciendo...";try{await(await b(async()=>{const{default:o}=await import("./vendor-DIPEJTOH.js").then(i=>i.d);return{default:o}},[])).default.delete("AniRD_DB"),window.location.reload()}catch(a){alert("Error al restablecer la base de datos: "+a.message),window.location.reload()}}}),document.getElementById("logout-btn").addEventListener("click",()=>g.logout())}};r.forEach(s=>{s.addEventListener("click",()=>d(s.dataset.tab))});const f=document.getElementById("sync-btn-profile-mobile");f&&f.addEventListener("click",async()=>{const s=f.innerHTML;f.innerHTML="Sincronizando...";try{const n=await h.getAllData();await g.syncWithServer(n),f.innerHTML="¡Sincronizado con éxito!",setTimeout(()=>{window.location.reload()},1500)}catch(n){f.innerHTML=`Error: ${n.message}`,setTimeout(()=>{f.innerHTML=s},3e3)}}),await d("list")}}const D=Object.freeze(Object.defineProperty({__proto__:null,default:T},Symbol.toStringTag,{value:"Module"}));export{D as P,x as s};
