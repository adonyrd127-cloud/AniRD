import{a as $,d as L,b}from"./page-homepage-HfrhAw2J.js";import"./vendor-DIPEJTOH.js";class A{constructor(e){this.params=e,this.animeId=parseInt(e.id),this.episodeNum=parseInt(e.ep)||1,this.lang=e.lang||"sub",this.anime=null,this.localInfo=null,this.episodeData=null,this.relatedAnimes=[],this.anilistEpisodes=[],this.isFav=!1,this.watchedEpisodes=new Set,this.isTheater=localStorage.getItem("watch-theater-mode")==="true",this.sortDesc=!1,this.searchQuery=""}async render(){var d,p,m,x,w,r;try{console.log("Iniciando carga de WatchPage Premium para ID:",this.animeId,"Episodio:",this.episodeNum);const n=await $.getAnimeInfo(this.animeId);n&&n.data&&(this.anime=n.data);const f=new URLSearchParams(window.location.search).get("title"),h=[];if(this.anime&&(h.push(this.anime.title),this.anime.title_english&&h.push(this.anime.title_english),this.anime.title_japanese&&h.push(this.anime.title_japanese),this.anime.title_synonyms&&h.push(...this.anime.title_synonyms)),f&&!h.includes(f)&&h.push(f),h.length>0){let v=null;for(const u of h){const y=await $.searchLocal(u);if(y&&y.success&&y.data&&y.data.results&&y.data.results.length>0){v=y;break}}if(v){const u=v.data.results.find(I=>h.some(E=>I.title.toLowerCase().includes(E.toLowerCase())))||v.data.results[0];this.anime||(this.anime={title:u.title,images:{jpg:{large_image_url:u.thumbnail}},genres:[],synopsis:"Cargado desde el servidor local de AniRD."});const y=await $.getAnimeInfo(u.url);if(y.success){this.localInfo=y.data;const I=this.localInfo.episodes.find(E=>E.number===this.episodeNum);if(I&&I.url){const E=await $.getEpisode(I.url);if(E.success&&E.data){this.episodeData=E.data;const C=this.episodeData.servers[this.lang]||this.episodeData.servers.sub||[];this.episodeData.activeServers=C}}}}}this.isFav=await L.isFavorite(this.animeId);const g=await b.history.where({animeId:String(this.animeId)}).toArray();this.watchedEpisodes=new Set(g.map(v=>Number(v.episodeId)))}catch(n){console.error("Error crítico al renderizar WatchPage Premium:",n)}const e=document.createElement("div");if(e.className="page-enter",!this.anime)return e.innerHTML=`
      ...
      `,e;document.title=`${this.anime.title} — Episodio ${this.episodeNum} (${this.lang.toUpperCase()}) — AniRD`;const t=this.watchedEpisodes.has(this.episodeNum),s=`https://anilist.co/search/anime?search=${encodeURIComponent(this.anime.title)}`,l=`https://myanimelist.net/anime/${this.anime.mal_id||""}`,a=((p=(d=this.anime.images)==null?void 0:d.jpg)==null?void 0:p.large_image_url)||"",o=((x=(m=this.anime.images)==null?void 0:m.jpg)==null?void 0:x.large_image_url)||a;let i="";if(this.anime.status==="Currently Airing"&&this.anime.broadcast&&this.anime.broadcast.time){const n=this.anime.broadcast,c={Sundays:0,Mondays:1,Tuesdays:2,Wednesdays:3,Thursdays:4,Fridays:5,Saturdays:6};if(c[n.day]!==void 0){const[f,h]=n.time.split(":").map(Number),g=new Date(new Date().toLocaleString("en-US",{timeZone:n.timezone||"Asia/Tokyo"}));let v=new Date(g);v.setHours(f,h,0,0);let u=c[n.day]-g.getDay();(u<0||u===0&&v<g)&&(u+=7),v.setDate(v.getDate()+u),v-g>0&&(i=`
    ...
    `,e}async afterRender(){if(this._initPlayerControls(),this._initServerPills(),this._initSynopsisExpand(),this._initWatchedToggleControls(),this._loadEnrichedEpisodesAndRecommendations(),window.activeWatchInterval&&(clearInterval(window.activeWatchInterval),window.activeWatchInterval=null),this.anime){const e=this.watchedEpisodes.has(this.episodeNum),t=document.getElementById("btn-watched-status"),s=document.getElementById("watched-status-text");e?(t&&t.classList.add("active"),s&&(s.textContent="Visto")):(t&&t.classList.remove("active"),s&&(s.textContent="Marcar Visto"),this.watchTimeCounter=0,window.activeWatchInterval=setInterval(async()=>{var a,o;if(!document.getElementById("watch-layout")){clearInterval(window.activeWatchInterval),window.activeWatchInterval=null;return}if(!document.hidden&&(this.watchTimeCounter++,this.watchTimeCounter>=120)){clearInterval(window.activeWatchInterval),window.activeWatchInterval=null,console.log("[WatchTimer] 2 minutos cumplidos. Marcando como visto automáticamente.");const i=this.anime?{animeTitle:this.anime.title,animeCover:((o=(a=this.anime.images)==null?void 0:a.jpg)==null?void 0:o.large_image_url)||this.anime.cover||"",animeType:this.anime.type||"",animeScore:this.anime.score||""}:{};await L.addToHistory(String(this.animeId),this.episodeNum,120,120,i),this.watchedEpisodes.add(this.episodeNum),t&&t.classList.add("active"),s&&(s.textContent="Visto"),this.renderEpisodes&&this.renderEpisodes()}},1e3))}}_initPlayerControls(){const e=document.getElementById("watch-layout"),t=document.getElementById("player-section"),s=document.getElementById("main-column"),l=document.getElementById("dim-overlay"),a=document.getElementById("btn-theater"),o=document.getElementById("btn-lights"),i=document.getElementById("btn-favorite"),d=document.getElementById("lights-text"),p=document.getElementById("theater-text"),m=document.getElementById("fav-text"),x=r=>{r?(e.classList.add("theater-active"),e.insertBefore(t,e.firstChild),p&&(p.textContent="Modo Normal")):(e.classList.remove("theater-active"),s.insertBefore(t,s.firstChild),p&&(p.textContent="Modo Cine"))};this.isTheater&&x(!0),a&&a.addEventListener("click",()=>{this.isTheater=!this.isTheater,localStorage.setItem("watch-theater-mode",this.isTheater),a.classList.toggle("active",this.isTheater),x(this.isTheater)});const w=r=>{const n=r!==void 0?r:!l.classList.contains("active");l.classList.toggle("active",n),t.classList.toggle("dimmed-active",n),o.classList.toggle("active",n),d&&(d.textContent=n?"Encender Luces":"Apagar Luces")};o&&o.addEventListener("click",()=>w()),l&&l.addEventListener("click",()=>w(!1)),i&&i.addEventListener("click",async()=>{if(!this.anime)return;const r=await L.toggleFavorite(this.anime);this.isFav=r,i.classList.toggle("active",r),m&&(m.textContent=r?"Quitar Favorito":"Favorito")})}_initServerPills(){const e=document.querySelectorAll(".server-pill-v5"),t=document.querySelector(".video-wrapper-v5 iframe");e.forEach(l=>{l.addEventListener("click",a=>{e.forEach(i=>i.classList.remove("active")),l.classList.add("active");const o=l.getAttribute("data-url");if(t&&o){t.src=o;const i=document.getElementById("video-container");i.style.opacity="0.5",setTimeout(()=>i.style.opacity="1",500)}})}),document.querySelectorAll(".lang-pill-v5").forEach(l=>{l.addEventListener("click",a=>{const o=l.getAttribute("data-lang");this.anime&&`${encodeURIComponent(this.anime.title)}`,window.location.href=`/watch/${this.animeId}/${this.episodeNum}/${o}?title=${encodeURIComponent(this.anime.title)}`})})}_initSynopsisExpand(){const e=document.getElementById("synopsis-box"),t=document.getElementById("btn-more-synopsis");t&&e&&t.addEventListener("click",()=>{const s=e.classList.toggle("expanded");t.textContent=s?"... ver menos":"... ver más"})}_startCountdownTimer(e,t){let s=e-t;const l=setInterval(()=>{const a=document.getElementById("countdown-timer");if(!a){clearInterval(l);return}if(s-=1e3,s<=0){a.textContent="¡Disponible ya en Emisión!",clearInterval(l);return}const o=Math.floor(s/864e5),i=Math.floor(s%864e5/36e5),d=Math.floor(s%36e5/6e4),p=Math.floor(s%6e4/1e3);let m="";o>0&&(m+=`${o}d `),(i>0||o>0)&&(m+=`${i}h `),m+=`${d}m ${p}s`,a.textContent=m},1e3)}async _loadEnrichedEpisodesAndRecommendations(){var a,o;const e=document.getElementById("related-grid"),t=document.getElementById("sidebar-ep-list"),[s,l]=await Promise.all([$.getAnimeRecommendations(this.animeId).catch(()=>null),$.getAnilistEpisodes(this.animeId).catch(()=>[])]);if(e)if(s&&s.data&&s.data.length>0){const i=s.data.slice(0,6);e.innerHTML=i.map(d=>{var p,m;return`
        ...
        `}).join("")}else e.innerHTML='<p style="color:var(--text-muted); font-size:12px; font-weight:600;">No hay recomendaciones similares disponibles.</p>';if(this.localInfo&&this.localInfo.episodes){const i=this.localInfo.episodes,d=((o=(a=this.anime.images)==null?void 0:a.jpg)==null?void 0:o.large_image_url)||"",p=this.anime.title,m=()=>{let r=[...i];if(this.sortDesc&&r.reverse(),this.searchQuery.trim()!==""&&(r=r.filter(c=>String(c.number).includes(this.searchQuery)||c.title&&c.title.toLowerCase().includes(this.searchQuery.toLowerCase()))),r.length===0){t.innerHTML='<p style="color:var(--text-muted); text-align:center; padding:20px; font-size:11px;">No se encontraron episodios.</p>';return}t.innerHTML=r.map(c=>{let f=`Episodio ${c.number}`,h=d;const g=l[c.number-1];if(g&&(g.title&&(f=g.title.replace(/^Episode \d+\s*-?\s*/i,"")),g.thumbnail&&(h=g.thumbnail)),c.number===this.episodeNum){const C=document.getElementById("active-episode-title");C&&(C.textContent=`${this.anime.title} — ${f}`)}const v=c.number===this.episodeNum,u=this.watchedEpisodes.has(c.number),y=`/watch/${this.animeId}/${c.number}/${this.lang}?title=${encodeURIComponent(p)}`,I=u?'<div class="ep-watched-badge-v5">✓ Visto</div>':"",E=u?'<div class="ep-progress-bar-v5"><div class="ep-progress-fill-v5"></div></div>':"";return`
          ...
          `}).join("");const n=document.getElementById("btn-watched-all");if(n&&this.localInfo&&this.localInfo.episodes){const f=this.localInfo.episodes.map(h=>h.number).every(h=>this.watchedEpisodes.has(h));n.classList.toggle("active",f),n.title=f?"Desmarcar toda la temporada":"Marcar toda la temporada como vista"}};this.renderEpisodes=m,m();const x=document.getElementById("ep-search-input");x&&x.addEventListener("input",r=>{this.searchQuery=r.target.value,m()});const w=document.getElementById("btn-sort-ep");w&&w.addEventListener("click",()=>{this.sortDesc=!this.sortDesc,w.classList.toggle("active",this.sortDesc),m()})}else t&&(t.innerHTML=`
