import{d as s,a as f}from"./page-homepage-BLqD5dcZ.js";import"./vendor-Cmt3X8aB.js";class b{async render(){const i=document.createElement("div");return i.innerHTML=`
      <style>
        .page-container {
    ...
    `,i}async afterRender(){const i=document.getElementById("history-results-grid"),d=document.getElementById("favorites-results-grid"),l=document.getElementById("theme-select"),c=document.getElementById("lang-select"),u=await s.getSetting("theme","dark"),p=await s.getSetting("lang","SUB");l.value=u,c.value=p,l.addEventListener("change",async e=>{const t=e.target.value;await s.setSetting("theme",t),t==="light"?document.body.classList.add("light-theme"):document.body.classList.remove("light-theme")}),c.addEventListener("change",async e=>{await s.setSetting("lang",e.target.value)});const g=await s.getContinueWatching();if(g.length===0)i.innerHTML='<div class="empty-state">No tienes historial de reproducción aún.</div>';else{i.innerHTML="";for(const e of g)try{const r=(await f.getAnimeInfo(e.animeId)).data,a=document.createElement("anime-card");a.setAttribute("mode","thumbnail"),a.data={...r,currentEpisode:e.episodeId},i.appendChild(a)}catch(t){console.error(t)}}const m=await s.getFavorites();m.length===0?d.innerHTML='<div class="empty-state">Aún no has guardado ningún anime en favoritos.</div>':(d.innerHTML="",m.forEach(e=>{const t=document.createElement("anime-card");t.data={mal_id:e.animeId,title:e.title,images:{jpg:{large_image_url:e.cover}},type:e.type,score:e.score},d.appendChild(t)}));const n=document.getElementById("following-results-grid"),o=await s.getFollowing();o.length===0?n.innerHTML='<div class="empty-state">No estás siguiendo ningún anime todavía.</div>':(n.innerHTML="",o.forEach(e=>{const t=document.createElement("div");t.style.position="relative";const r=document.createElement("anime-card");r.data={mal_id:e.animeId,title:e.title,images:{jpg:{large_image_url:e.cover}}};const a=document.createElement("div");a.className="timer-overlay",a.innerHTML=this.calculateCountdown(e.broadcast),t.appendChild(r),t.appendChild(a),n.appendChild(t)}))}calculateCountdown(i){if(!i||!i.day||!i.time)return"<span>Siguiente Ep: --:--</span>";try{const l=["Sundays","Mondays","Tuesdays","Wednesdays","Thursdays","Fridays","Saturdays"].indexOf(i.day),[c,u]=i.time.split(":").map(Number),p=new Date,g=9*60,m=p.getTimezoneOffset(),n=new Date(p.getTime()+(g+m)*6e4);let o=l-n.getDay();o<0&&(o+=7);const e=new Date(n);e.setDate(n.getDate()+o),e.setHours(c,u,0,0),o===0&&e<n&&e.setDate(e.getDate()+7);const t=e-n,r=Math.floor(t/(1e3*60*60)),a=Math.floor(r/24),v=r%24;return a>0?`<span>⏳ ${a}d ${v}h</span>`:`<span>⏳ ${v}h</span>`}catch{return"<span>Pronto</span>"}}}export{b as default};
