...
export default class AnimeDetailPage {
  ...
  async render() {
    ...
    if (!this.anime) {
      ...
      errorContainer.className = 'page-enter';
    ...
    // SEO: Dynamic title
    document.title = `${this.anime.title_english || this.anime.title || 'Anime'} — AniRD`;
  ...
  async afterRender() {
    ...
    const closeSlideOver = () => {
      ...
      const relPanel = document.querySelector('.relations-panel');
      if (panel && overlay) {
         panel.style.animation = 'slideOut 0.3s forwards';
         overlay.style.animation = 'fadeOut 0.3s forwards';
         if (relPanel) relPanel.style.animation = 'fadeOut 0.3s forwards';
  ...
  renderRelationsHtml() {
    if (!this.relations || this.relations.length === 0) {
      return `<div class="no-relations-message">No se encontraron precuelas, secuelas u otras conexiones.</div>`;
    }

    let html = '';
    
    // Sort so Prequels and Sequels appear first
    const sortedRelations = [...this.relations].sort((a, b) => {
      const relA = a.relation.toLowerCase();
      const relB = b.relation.toLowerCase();
      if (relA === 'prequel' || relA === 'sequel') return -1;
      if (relB === 'prequel' || relB === 'sequel') return 1;
      return 0;
    });

    for (const rel of sortedRelations) {
      const relationName = rel.relation;
      const badgeClass = relationName.toLowerCase() === 'prequel' ? 'prequel' 
                       : relationName.toLowerCase() === 'sequel' ? 'sequel' 
                       : 'other';

      const localizedRelation = relationName === 'Prequel' ? 'Precuela'
                              : relationName === 'Sequel' ? 'Secuela'
                              : relationName === 'Alternative version' ? 'Versión Alternativa'
                              : relationName === 'Side story' ? 'Historia Paralela'
                              : relationName === 'Spin-off' ? 'Spin-off'
                              : relationName === 'Parent story' ? 'Historia Principal'
                              : relationName === 'Summary' ? 'Resumen'
                              : relationName;

      for (const entry of rel.entry) {
        const isAnime = entry.type === 'anime';
        const url = isAnime ? `/anime/${entry.mal_id}` : entry.url;
        const target = isAnime ? 'data-link' : 'target="_blank" rel="noopener noreferrer"';

        html += `
          <a href="${url}" ${target} class="relation-item-card">
            <span class="relation-badge ${badgeClass}">${localizedRelation}</span>
            <div class="relation-item-title">${entry.name}</div>
            <div class="relation-item-meta">${entry.type.toUpperCase()}</div>
          </a>
        `;
      }
    }
