...
export default class AnimeDetailPage {
  ...
  async afterRender() {
    ...
    const renderEpisodes = async () => {
      ...
      let epCount = null;
      tabPanel.innerHTML = '<div style="color:white;text-align:center;padding:40px;">Cargando episodios...</div>';
      ...
      cachedEpCount = epCount || 12; // Fallback final
      renderGrid(cachedEpCount, watchedSet);
    };
    ...
    try {
      ...
      if (rels && rels.data) {
        rels.data.forEach(rel => {
          const label = rel.relation === 'Prequel' ? 'Precuela' : (rel.relation === 'Sequel' ? 'Secuela' : rel.relation);
          rel.entry.forEach(entry => {
            if (entry.type === 'anime') {
              const box = document.createElement('div');
              box.innerHTML = `<span class="rel-label">${label}</span><anime-card></anime-card>`;
